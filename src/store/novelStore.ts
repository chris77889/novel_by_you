...
export const useNovelStore = create<NovelStore>((set) => ({
  ...
  addHistory: (history) => {
    ...
    console.log(`[${new Date().toISOString()}] addHistory called (session only).`, historyWithId);
    set(state => ({
  ...
  updateHistory: (id, updates) => {
    ...
    console.log(`[${new Date().toISOString()}] updateHistory called for ID: ${id} (session only).`, updates);
    set(state => ({
